...
public:
  ...
  int initialize(std::string path) {
    if (sqlite3_open(path.c_str(), &db) != SQLITE_OK) {
      spdlog::error("Open database with error: {}", sqlite3_errmsg(db));
      return DATABASE_OPEN_ERROR;
    }
    char *zErrMsg = nullptr;
    for (auto sql : CreateSentence) {
      spdlog::info("Initialize sql: {}", sql);
      if (sqlite3_exec(db, sql.c_str(), nullptr, nullptr, &zErrMsg) != SQLITE_OK) {
  ...
  int create_user(github_user user) {
    sqlite3_stmt *stmt;

    std::string create_sql = "";
    std::string query_sql  = "SELECT `id` FROM `users` WHERE `id` = ?";

    int code = sqlite3_prepare_v2(db, query_sql.c_str(), -1, &stmt, 0);
    if (code != SQLITE_OK) {
    ...
    code = sqlite3_prepare_v2(db, create_sql.c_str(), -1, &stmt, 0);
    if (code != SQLITE_OK) {
      sqlite3_finalize(stmt);
      return code;
    }
