...
RUN set -eux && sed -i "s/deb.debian.org/mirrors.aliyun.com/g" /etc/apt/sources.list.d/debian.sources && \
  ...
  git clone https://github.com/microsoft/vcpkg-tool vcpkg-tool --depth 1 --branch 2024-10-18 --single-branch && \
  cd vcpkg-tool && mkdir build && cd build && cmake -DBUILD_TESTING=OFF .. && cmake --build . --target install && \
  cd /workspace && git clone https://github.com/microsoft/vcpkg.git --depth 1 --branch 2024.09.30 --single-branch && \
  cd code && make deps
